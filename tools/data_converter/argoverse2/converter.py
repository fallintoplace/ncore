...
class Argoverse2Converter4Config(FileBasedDataConverterConfig):
    ...
    # Lidar model: "nominal" derives a VLP-32C structured model (per unit) and stores
    # per-point model elements + intrinsics; "none" stores raw ray bundles only.
    lidar_model_source: Literal["nominal", "none"] = "nominal"
    ...
    def _decode_lidars(
        ...
        if self._lidar_model_source == "nominal":
            ...
            for unit, mask_first in (
                ...
                # ego -> unit sensor frame, then decompensate to per-point time so the
                # firing geometry is clean (un-smeared by ego motion).
                T_ego_unit = se3_inverse(T_unit_ego[unit])
                xyz_sensor_first = (T_ego_unit[:3, :3] @ first_sweep.xyz[mask_first].T).T + T_ego_unit[:3, 3]
                xyz_decomp_first = compensators[unit].motion_decompensate_points(
                    sensor_id=unit,
                    xyz_reftime=xyz_sensor_first.astype(np.float32),
                ...
                self.logger.info(
                    f"Derived VLP-32C model for {unit}: {geometry.n_columns} columns, "
        ...
        for i, ts_ns in enumerate(tqdm.tqdm(sweep_ts_ns, desc="Process lidar")):
            ...
            for unit in lidar_ids:
                ...
                xyz_raw = compensators[unit].motion_decompensate_points(
                    sensor_id=unit,
                    xyz_reftime=xyz_sensor.astype(np.float32),
                ...
                # Per-point structured-model element (row, column) from beam + firing time.
                model_element = None
                if unit in unit_geometry:
        ...
        for unit, model in unit_model.items():
            intrinsics_writer.store_lidar_intrinsics(lidar_id=unit, lidar_model_parameters=model)
...
@click.option(
    "lidar_model_source",
    "--lidar-model-source",
    type=click.Choice(["nominal", "none"], case_sensitive=False),
    default="nominal",
    show_default=True,
    help="Lidar model: 'nominal' derives a VLP-32C structured model per unit (model "
    "elements + intrinsics); 'none' stores raw ray bundles only.",
)
@click.pass_context

...
- **Lidar**: two stacked Velodyne VLP-32C units (`up_lidar` / `down_lidar`, 10 Hz).
  ...
  - A structured VLP-32C model is stored per unit as lidar intrinsics, with
    ...
    in their own frames (one `cw`, one `ccw`); this is detected from the data.
    Deriving from the decompensated cloud (not the ego-motion-smeared compensated
    one) plus empirical per-row offsets gives sub-0.1 deg far-range reconstruction,
    on par with native-column sensors. Pass `--lidar-model-source none` to store raw
    ray bundles only.

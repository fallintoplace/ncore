...
``cw``, one ``ccw``), which is detected from the data. Deriving from the
decompensated cloud (not the ego-motion-smeared compensated one) plus empirical
per-row offsets gives sub-0.1 deg far-range reconstruction, on par with
native-column sensors. Pass ``--lidar-model-source none`` to store raw ray bundles
only.
